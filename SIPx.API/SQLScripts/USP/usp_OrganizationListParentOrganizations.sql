CREATE PROCEDURE [dbo].[usp_OrganizationListParentOrganizations] (@UserId nvarchar(450)) 
AS 
...
WHERE Organizations.StatusID = 1
	AND Organizations.OrganizationId IN (SELECT ParentOrganizationId FROM Organizations WHERE StatusId = 1)
