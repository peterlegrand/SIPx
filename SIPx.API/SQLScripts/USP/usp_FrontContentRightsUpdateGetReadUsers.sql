CREATE PROCEDURE usp_FrontContentRightsUpdateGetReadUsers (@UserId nvarchar(450), @ContentId int)
AS

DECLARE @IsRelationBasedReadSelectable bit, @IsProjectBasedReadSelectable bit, @IsOrganizationBasedReadSelectable bit, @IsFreeReadSelectable bit;
SELECT @IsRelationBasedReadSelectable = IsRelationBasedReadSelectable , @IsProjectBasedReadSelectable = IsProjectBasedReadSelectable , @IsOrganizationBasedReadSelectable = IsOrganizationBasedReadSelectable , @IsFreeReadSelectable = IsFreeReadSelectable
...
WHERE UserID IS NOT NULL 
	AND UserID <> @UserId
	AND UserID NOT IN (SELECT UserId FROM ContentUserRead WHERE ContentID = @ContentId)

END
ELSE
BEGIN

IF @IsRelationBasedReadSelectable =1
BEGIN
...
WHERE PersonRelationTypes.CanAssignOwnershipToFrom = 1
	...
	AND NewOwner.UserID NOT IN (SELECT UserId FROM ContentUserRead WHERE ContentID = @ContentId)
...
IF @IsOrganizationBasedReadSelectable = 1
BEGIN
...
WHERE CurrentOwnerRole.CanAssignContentOwnership = 1
	...
	AND Persons.UserID NOT IN (SELECT UserId FROM ContentUserRead WHERE ContentID = @ContentId)
END
...
SELECT DISTINCT UserId, Name FROM @UserTable
	WHERE UserID NOT IN (SELECT UserId FROM ContentUserRead WHERE ContentID = @ContentId)
