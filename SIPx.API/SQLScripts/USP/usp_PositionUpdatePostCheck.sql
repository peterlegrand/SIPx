CREATE PROCEDURE usp_PositionUpdatePostCheck (
	@PositionId int
	, @Name nvarchar(50)
...
IF  (SELECT COUNT(*) FROM Positions WHERE PositionID = @PositionId) <>1
BEGIN
	insert into @ErrorIdsTable values(27)
END

IF  (SELECT COUNT(*) FROM PositionLanguages WHERE LanguageId = @LanguageID AND PositionLanguages.Name = @Name) >0
BEGIN
