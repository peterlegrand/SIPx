CREATE PROCEDURE usp_ProcessTypeFlowPassUpdatePostCheck (
...
IF (SELECT COUNT(*) FROM ProcessTypeFlowPasses WHERE ProcessTypeFlowPassId = @ProcessTypeFlowPassId) = 0 
BEGIN
insert into @ErrorIdsTable values(90)
END

IF (SELECT MAX(Sequence) 
	FROM ProcessTypeFlowPasses ) < @Sequence + 1
BEGIN
...
IF (SELECT COUNT(*) FROM ProcessTypeFlowPassTypes WHERE ProcessTypeFlowPassTypeId = @ProcessTypeFlowPassTypeId) = 0 
BEGIN
...
IF (SELECT COUNT(*) FROM ProcessTypeFlowPassComparisonOperators WHERE ProcessTypeFlowPassComparisonOperators.ProcessTypeFlowPassComparisonOperatorID = @ComparisonOperatorId) = 0 OR @ComparisonOperatorId IS NOT NULL
BEGIN
