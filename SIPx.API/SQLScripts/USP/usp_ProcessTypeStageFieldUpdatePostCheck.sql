CREATE PROCEDURE usp_ProcessTypeStageFieldUpdatePostCheck (
...
IF (SELECT COUNT(*) FROM ProcessTypeStageFields WHERE ProcessTypeStageFieldId = @ProcessTypeStageFieldId) = 0 
BEGIN
insert into @ErrorIdsTable values(113)
END

IF (SELECT COUNT(*) FROM ProcessTypeStageFieldStatuses WHERE ProcessTypeStageFieldStatusId = @ProcessTypeStageFieldStatusId) = 0 
BEGIN
